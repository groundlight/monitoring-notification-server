...
export const Dropdown = ({ options, selected, setSelected, valid, onChange, className }:
    { options: any[], selected: string, setSelected: (e: string, idx: number) => void, valid?: boolean, onChange?: (open: boolean) => void, className?: string }
) => {
        ...
        <div className="relative inline-block text-left w-full place-self-center">
            <div>
                <span className="rounded-md shadow-sm">
                    <button type="button" className={`${className} inline-flex justify-center w-full rounded-md border ${valid !== false ? "border-gray-300" : "border-red-500"} px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50" id="options-menu`} aria-haspopup="true" aria-expanded="true" onClick={() => {
